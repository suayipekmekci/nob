...
const ModalSignIn = React.createClass({
    ...
    clickSignIn(event) { // burada da tooken geliyor direk giriş olacak
      ...
      PostModule.verificationPostType("http://52.48.0.60/api/user/register",postParams).then((successMessage) => {
        if(successMessage.access_token){
          if(successMessage.access_token !== ""){
            location.hash = "/";
            location.reload();
                    ...
                    <Modal.Body>
                        <form onSubmit={this.clickSignIn}>
                            <Col sm={12}>
                                <div className="t-a-c m-b-15 m-t-15">
                                    <img className="m-b-15" src={require("../../../common/images/nobitsyLogo.png")}/>
                                    <div className="modalTitle">Complete your Nobitsy user profile.</div>
                                </div>
                                <Row className="m-b-15">
                                    <Col componentClass={ControlLabel} sm={2}>
                                        Email:
                                ...
                                <Row className="m-b-15">
                                    ...
                                    <Col sm={10} className="p-0">
                                        <Col sm={6}>
                                            <FormControl type="text" name="name" id="userName" placeholder="First name"/>
                                        </Col>
                                        <Col sm={6}>
                                            <FormControl type="text" name="surname" id="userSurname" placeholder="Last name"/>
                                        </Col>
                                    </Col>
                                </Row>
                            </Col>
...
const MyComponent = React.createClass({
    ...
    componentWillMount() {
      ...
      PostModule.postType("http://52.48.0.60/api/user/checkToken",postParams).then((successMessage) => {
        console.log(successMessage.message);
        if(successMessage.message == "ok"){
          this.changeModal(0)
          document.getElementById("emailAccount").value = successMessage.email;
          document.getElementById("userName").value = successMessage.name;
          document.getElementById("userSurname").value = successMessage.surname;
        }else{
          this.changeModal(2)
        }
      });
